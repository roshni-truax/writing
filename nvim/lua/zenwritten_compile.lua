...
--   ../palette/zenwritten.json         the grounds and inks, for the
--                                      programs here that are neither
--                                      neovim nor wezterm: printer and viewer
--
...
-- The two inks that are a choice rather than a consequence. Everything else
-- in the palette falls out of the theme - the grounds are Normal's, and
-- `dim` is the terminal's last bright - but these two were picked by eye
-- against those grounds and are not a mix of them (measured: the nearest
-- oklab blend misses both by a few units a channel, and by a different
-- amount in each background). So they are written down, here with the rest
-- of the theme's configuration, and carried out to whatever reads them.
--
--   faint   just off the ground: rules, an empty bar, slide numbers
--   status  the viewer's status line, mixed to sit on the terminal's ground
local INKS = {
  dark = { faint = "#4E4849", status = "#6E6466" },
  light = { faint = "#B8B5B5", status = "#969192" },
}
...
function M.compile()
  ...
  local repo = require("writing").repo()
  ...
  -- The palette as plain data, for the programs here that read neither lua
  -- nor a colorscheme: printer sets a deck in it and viewer tints its status
  -- line with it. They held their own copies of these colours until
  -- 2026-09-09, which a theme change could only leave behind.
  local rows = {}
  for _, bg in ipairs({ "dark", "light" }) do
    local set = sets[bg]
    rows[#rows + 1] = ("  %q: {"):format(bg)
    rows[#rows + 1] = ('    "bg": %q,'):format(hex(set.hl.Normal.bg))
    rows[#rows + 1] = ('    "fg": %q,'):format(hex(set.hl.Normal.fg))
    rows[#rows + 1] = ('    "dim": %q,'):format(set.terminal[16])
    rows[#rows + 1] = ('    "faint": %q,'):format(INKS[bg].faint)
    rows[#rows + 1] = ('    "status": %q'):format(INKS[bg].status)
    rows[#rows + 1] = bg == "dark" and "  }," or "  }"
  end
  write(repo .. "/palette/zenwritten.json", vim.list_extend({
    "{",
    '  "_": "Generated by nvim/lua/zenwritten_compile.lua - do not edit by hand; '
      .. 'change the configuration there and recompile instead.",',
  }, vim.list_extend(rows, { "}" })))

  vim.notify("compiled zenwritten into colors/, lua/lualine/themes/, wezterm/ and palette/")
end
